    ...
    return (
      <div>
        ...
        >
          ...
          <ProgressPanel
            key="hours"
            text={`${this.floor(this.state.hours)}小时`}
            percent={(hours / 24) * 100}
          />
          <ProgressPanel
            key="minutes"
            text={`${this.floor(this.state.minutes)}分钟`}
            percent={(minutes / 60) * 100}
          />
          <ProgressPanel
            key="seconds"
            text={`${this.floor(this.state.seconds)}秒`}
            percent={(seconds / 60) * 100}
          />
        </Tabs>
        <WhiteSpace />
        <ProgressPanel
          text={`${this.floor(days)}天${this.format(hours)}小时${this.format(
            minutes
          )}分钟${this.format(seconds)}秒`}
        />
      </div>
    );
  }
